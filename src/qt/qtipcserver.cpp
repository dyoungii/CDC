...
static bool ipcScanCmd(int argc, char *argv[], bool fRelay)
{
    ...
    {
        if (boost::algorithm::istarts_with(argv[i], "cloudcoin:"))
        {
...
static void ipcThread(void* pArg)
{
    ...
    RenameThread("cloudcoin-gui-ipc");
...
void ipcInit(int argc, char *argv[])
{
    ...
    try {
        ...
        // Make sure we don't lose any cloudcoin: URIs
        for (int i = 0; i < 2; i++)
        {
        ...
        // Make sure only one cloudcoin instance is listening
        message_queue::remove(BITCOINURI_QUEUE_NAME);
        delete mq;
